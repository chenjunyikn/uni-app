<template>
	<view scroll-y="true">
		<view class="h1">{{year}}年流程作业量对比图表展示</view>
			...
			getdata(option) {
				uni.getStorage({
					...
					success: (res) => {
						...
						uni.request({
							url: this.url + 'jobAmount/flowAmount',
							data: {
								compid: this.compid,
								year: this.year
							},
							success: (res) => {
								for (var name of res.data.rows) {
									...
									this.compdata1.push(name['zk-101']);
									this.compdata2.push(name['zk-102']);
									this.compdata3.push(name['zk-103']);
									this.compdata4.push(name['zk-104']);
									this.compdata5.push(name['zk-201']);
									this.compdata6.push(name['zk-202']);
								}
